...
const SalesDashboard = () => {
  ...
  const [ScheduleEnquiry, setScheduleEnquiry] = useState([]);
  const [ReScheduleENquiry, setReScheduleENquiry] = useState([]);
  const [nondisbursedleads, setnondisbursedleads] = useState([])
  ...
  useEffect(() => {
    if (leads.length && applications.length) {
      ...
      lastMonthStart.setMonth(lastMonthStart.getMonth() - 1, 1); // First day of last month
      ...
      lastYearStart.setFullYear(lastYearStart.getFullYear() - 1, 0, 1); // January 1st of last year
      lastYearStart.setHours(0, 0, 0, 0);

      const lastYearEnd = new Date();
      lastYearEnd.setFullYear(lastYearEnd.getFullYear() - 1, 11, 31); // December 31st of last year
      lastYearEnd.setHours(23, 59, 59, 999);

      // Filter Last Month leads and applications
      const lastMonthLeads = leads.filter(lead =>
        new Date(lead.createdTime) >= lastMonthStart &&
        new Date(lead.createdTime) <= lastMonthEnd
      );

      const lastMonthApplications = applications.filter(application =>
        new Date(application.createdTime) >= lastMonthStart &&
        new Date(application.createdTime) <= lastMonthEnd
      );

      // Filter Last Year leads and applications
      const lastYearLeads = leads.filter(lead =>
        new Date(lead.createdTime) >= lastYearStart &&
        new Date(lead.createdTime) <= lastYearEnd
      );

      const lastYearApplications = applications.filter(application =>
        new Date(application.createdTime) >= lastYearStart &&
        new Date(application.createdTime) <= lastYearEnd
      );

      // Update state with filtered data
      setLeadsLastMonthYear({ lastMonth: lastMonthLeads, lastYear: lastYearLeads });
      setApplicationsLastMonthYear({ lastMonth: lastMonthApplications, lastYear: lastYearApplications });
    }
  }, [leads, applications]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      //   const statusName = lead?.leadStatus?.leadStatusName?.trim()?.toLowerCase();
      //   return statusName !== "sent to los";
      // });
  ...
  useEffect(() => {
    if (DisbursedCase && leads) {
      const disbursedAppIds = new Set(DisbursedCase.map(item => item.applicationNo));
      // Step 2: Filter the leads array
      const filteredLeads = leads.filter(lead => !disbursedAppIds.has(lead.appId));
      setnondisbursedleads(filteredLeads)
  ...
  // useEffect(() => {
  //   if (leads && applications && DisbursedCase && RejectedCase) {
  //     getCounts();
  //   }
  // }, [leads, applications, DisbursedCase, RejectedCase]);

  const getCounts = useCallback(() => {
    if (isLoading) {
      setCounts({ leadsCount: 0, applicationsCount: 0, disbursedCount: 0, rejectedCount: 0 });
      return;
    }
    ...
    let leadsCount = 0, applicationsCount = 0, disbursedCount = 0, rejectedCount = 0;

    if (selectedTab === 'Last Month') {
      leadsCount = lastMonthLeads.length;
      applicationsCount = lastMonthApplications.length;
      disbursedCount = lastMonthApplications.filter(app => app.stage === 'Disbursed').length;
      rejectedCount = lastMonthApplications.filter(app => app.stage === 'Rejected').length;
    } else if (selectedTab === 'Last Year') {
      leadsCount = lastYearLeads.length;
      applicationsCount = lastYearApplications.length;
      disbursedCount = lastYearApplications.filter(app => app.stage === 'Disbursed').length;
      rejectedCount = lastYearApplications.filter(app => app.stage === 'Rejected').length;
    } else {
      leadsCount = forSales?.length || 0;
      applicationsCount = forCredits?.length || 0;
      disbursedCount = DisbursedCase?.length || 0;
      rejectedCount = RejectedCase?.length || 0;


    }

    setCounts({ leadsCount, applicationsCount, disbursedCount, rejectedCount });
  }, [leads, applications, selectedTab, isLoading]);

  // Call getCounts when dependencies update
  useEffect(() => {
    if (!isLoading) {
      getCounts();

    }
  }, [finalApplication, DisbursedCase, RejectedCase, isLoading]);
  ...
  //             </View>
  //           </View>
  //           <View style={styles.headerShadowLine} />
  //           {/* Tabs */}
  ...
  //                     Both Rejected Leads and Application data are available.
  //                   </Text>
  ...
  //                     }}
  //                   >
  //                     <Text style={styles.modalButtonText}>Go to Rejected</Text>
  //                   </TouchableOpacity>
  ...
  //                     }}
  //                   >
  //                     <Text style={styles.modalButtonText}>Go to Leads</Text>
  //                   </TouchableOpacity>
  ...
  //           {/* Summary Grid */}
  //           {isLoading ? (
  //             <ActivityIndicator size="large" color="#007bff" style={{ marginTop: verticalScale(50) }} />
  //           ) : (
  //             <ScrollView
  ...
  //                   {row.map((item, index) => (
  //                     <View style={styles.cardWrapper} key={`${rowIndex}-${index}`}>
  //                       <Animatable.View
  //                         animation="fadeInUp"
  ...
  return (
    ...
    //             style={[
    //               styles.row,
    //               isSingle && { justifyContent: "center" } // Center single card
    //             ]}
    //             key={rowIndex}
    //           >
    //             {row.map((item, index) => (
    ...
    //                 style={[
    //                   styles.cardWrapper,
    //                   row.length === 1 && { width: width * 0.6 } // bigger and centered
    //                 ]}
    //                 key={`${rowIndex}-${index}`}
    //               >
    //                 <Animatable.View
    ...
    //               pipeline: [...leads, ...finalApplicationInp],  // merged array
    //             })
    //           }
    //         >
    //           <LinearGradient
    ...
    //               {/* {formatTimeAgo(item.lastModifiedTime)} · {item.stage} */}
