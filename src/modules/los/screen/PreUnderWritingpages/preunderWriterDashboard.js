...
const FloatingFAB = React.memo(({ isVisible, toggle, navigation, isDisabled }) => {
  return (
        ...
        <View style={styles.glassFabContainer}>
          ...
          <View style={styles.glassFab}>
            ...
            <View style={styles.fabContent}>
              ...
              >
                <Image source={require("../../asset/mic.png")} style={styles.icon} />
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                disabled={isDisabled}
              >
                <Image source={require("../../asset/bell.png")} style={styles.icon} />
              </TouchableOpacity>
...
const PreUnderWriterDashboard = () => {
  ...
  useEffect(() => {
    ...
    setrecentActivityfilter(filterUserActivity(logsData, userDetails.userName));
  ...
  const handleInProgressPress = useCallback(() => {
    ...
    const grouped = groupStageItems(source, 'Lead');
  ...
  // 📊 SUMMARY CARDS
  // =========================
  const isDisabled = !AllDataofApplication || AllDataofApplication.length === 0;
  const canAccessFab = useMemo(() => {
    return userDetails?.designation === "CEO";
  }, [userDetails]);
  ...
  const formatTimeAgo = timestamp =>
    formatBusinessRelativeTime(timestamp, BusinessDate);
  ...
  const commandStageSource = useMemo(
    () =>
      userDetails?.designation === 'CEO'
        ? TI || []
        : [
  ...
  const totalTaskCount = useMemo(() => sumTaskBuckets(myTasks), [myTasks]);

  const commandStageBoard = useMemo(() => {
    return sortGroupedStages(groupStageItems(commandStageSource));
  }, [commandStageSource]);
  ...
  const commandCards = useMemo(() => {
    ...
    const softMap = {
      'Total Case': 'rgba(37,99,235,0.12)',
      'In Progress': 'rgba(14,165,233,0.12)',
      Disbursed: 'rgba(16,185,129,0.14)',
      Rejected: 'rgba(249,115,22,0.12)',
      Deviation: 'rgba(139,92,246,0.12)',
    };

    const captionMap = {
      'Total Case': 'Every case already mapped to your LOS decision lane.',
      'In Progress': 'Live stages waiting for movement or attention.',
      Disbursed: 'Closed and completed business from your desk.',
      Rejected: 'Sales and credit exceptions requiring review.',
      Deviation: 'Deviation worklist requiring fast handling.',
    };
    ...
    return summaryData.map(item => ({
      ...
      caption: captionMap[item.label] || 'Operational snapshot',
      accent: accentMap[item.label] || '#0F766E',
      iconBackground: softMap[item.label] || 'rgba(15,118,110,0.12)',
      accentSoft: softMap[item.label] || 'rgba(15,118,110,0.12)',
    }));
  }, [summaryData]);
    ...
    () => [
      {
        ...
        eyebrow: 'Track',
        title: 'Open Pipeline',
        subtitle: 'Jump into the merged pre-underwriting pipeline with one tap.',
      ...
      },
      {
        ...
        eyebrow: 'Orchestrate',
        title: 'Stage Command',
        subtitle: 'Open grouped stage routing and assign the next move quickly.',
      ...
      },
      {
        key: 'deviation',
        eyebrow: 'Resolve',
        title: 'Deviation Desk',
        subtitle: 'Review pending deviations and clear the exception queue.',
  ...
  const commandActivityItems = useMemo(
    () =>
      recentActivityfilter.slice(0, 5).map((item, index) => {
        ...
        const stagePresentation = getStagePresentation(stageText);
        ...
        return {
          ...
          badge: stageText,
          badgeColor: stagePresentation.badgeColor,
          badgeTextColor: stagePresentation.badgeTextColor,
    ...
    [formatTimeAgo, recentActivityfilter],
  );

  const heroPipelineCount =
    userDetails?.designation === 'CEO'
      ? TI.length
      : (Dashboardcount?.applicationInprogressDtoList?.length || 0) +
      (Dashboardcount?.leadInprogressList?.length || 0);
  ...
  return (
    <SafeAreaView style={styles.safeContainer}>
      ...
      <DashboardCommandCenter
        theme={losThemes.creditCommand}
        greeting={`${userDetails?.designation || 'Credit'} command center`}
        title={`${userDetails?.firstName || ''} ${userDetails?.lastName || ''}`.trim() || 'LOS User'}
        heroValue={heroPipelineCount}
        heroLabel={`${selectedTab} workflow load`}
        heroMeta={`Open tasks ${totalTaskCount} - Idle alerts ${filteredIdleList.length} - Deviations ${data.length}`}
        microStats={[
          { label: 'Cases', value: counts.leadsCount },
          { label: 'Applications', value: counts.applicationsCount },
          { label: 'Disbursed', value: counts.disbursedCount },
        ]}
        statusLabel={canAccessFab ? 'Executive View' : 'Underwriting Live'}
        statusTone={canAccessFab ? 'rgba(34,211,238,0.20)' : 'rgba(16,185,129,0.22)'}
        userInitials={userInitials.toUpperCase() || 'U'}
        tabs={['Overall', 'Last Month', 'Last Year']}
        ...
        topRightContent={
          canAccessFab ? (
            <TouchableOpacity
              style={styles.commandBellButton}
              onPress={() => setIdleReminderModalVisible(true)}
              activeOpacity={0.85}
            >
              <Image
                source={require('../../asset/bell.png')}
                style={styles.commandBellIcon}
              />
              {idleApplicationList.length > 0 ? (
                <View style={styles.commandBellBadge}>
                  <Text style={styles.commandBellBadgeText}>
                    {idleApplicationList.length}
                  </Text>
                </View>
              ) : null}
            </TouchableOpacity>
        ...
        spotlight={{
          ...
          title: `${selectedTab} underwriting snapshot`,
          value: `${totalTaskCount}`,
          description:
            'A live balance of case volume, application movement, disbursals, rejections, and team attention load.',
          distribution: [
            { label: 'Total Case', value: counts.leadsCount, color: '#2563EB' },
            { label: 'Applications', value: counts.applicationsCount, color: '#8B5CF6' },
            { label: 'Disbursed', value: counts.disbursedCount, color: '#10B981' },
            { label: 'Rejected', value: counts.rejectedCount, color: '#F97316' },
          ],
          metaLine: `${commandStageBoard.length} live stages are currently visible in the command grid.`,
        }}
        summaryTitle="Decision Deck"
        summaryCards={commandCards}
        quickActionsTitle="Fast Routes"
        quickActions={commandQuickActions}
        activityTitle="Recent Activity"
        activityItems={commandActivityItems}
        activityAction={{
          label: 'Open pipeline',
          onPress: () => {
            const pipelineData =
        ...
        loading={isLoading}
        contentBottomSpacing={canAccessFab ? 140 : 44}
      >
        <View style={styles.commandStageCard}>
          <View style={styles.commandStageHeader}>
            <View style={styles.commandStageTitleBlock}>
              <Text style={styles.commandStageEyebrow}>Stage Command</Text>
              <Text style={styles.commandStageTitle}>Where cases are stacked right now</Text>
            </View>

            <View style={styles.commandStageBadge}>
              <Text style={styles.commandStageBadgeText}>
                {commandStageBoard.length}
              </Text>
            </View>
          </View>

          <Text style={styles.commandStageCopy}>
            Tap any lane to drill into the exact applications or leads waiting in that stage.
          </Text>

          <View style={styles.commandStageGrid}>
            {commandStageBoard.slice(0, 6).map(([stage, apps]) => (
              <TouchableOpacity
                key={stage}
                activeOpacity={0.88}
                style={styles.commandStageChip}
                onPress={() => handleStageClick(apps)}
              >
                <Text style={styles.commandStageChipLabel} numberOfLines={2}>
                  {stage}
                </Text>
                <Text style={styles.commandStageChipValue}>{apps.length}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
            ...
            {filteredIdleList.length ? (
              filteredIdleList.map(app => (
                ...
                >
                  ...
                  <Text style={styles.idleAppText}>
                    Stage: {app.stage} ({app.descriptions?.map(d => `"${d}"`).join(', ')})
                  </Text>
                  <Text style={styles.idleAppText}>
                    User: {app.user}
                  </Text>
                  ...
                  <Text
                    style={[
                      styles.reminderFlag,
                      parseFloat(app.idleHours) > IDLE_THRESHOLD_HOURS &&
          ...
          <View style={styles.modalContent}>
            ...
            <FlatList
              ...
              renderItem={({ item: [stage, apps] }) => (
                <TouchableOpacity onPress={() => handleStageClick(apps)}>
...
const styles = StyleSheet.create({
  ...
  fabContent: {
    justifyContent: 'center',
    alignItems: 'center',
    gap: 16, // spacing between mic + bell
  ...
  commandBellIcon: {
    width: 22,
    height: 22,
    tintColor: losColors.text.inverse,
  },
  commandBellBadge: {
    position: 'absolute',
    top: 4,
  ...
  commandStageTitleBlock: {
    flex: 1,
    paddingRight: 12,
  },
  commandStageEyebrow: {
    color: losColors.brand[300],
    fontSize: 11,
    fontWeight: '800',
  ...
  commandStageChipLabel: {
    color: 'rgba(255,255,255,0.74)',
    fontSize: 12,
    fontWeight: '700',
    lineHeight: 18,
  },
  commandStageChipValue: {
    marginTop: 10,
    color: '#FFFFFF',
    fontSize: 20,
    fontWeight: '800',
